...
4. Put each reply command in its own list item, with one command and its result per item. Never join multiple reply actions with semicolons.
...
For the native Plan boundary, follow `$task-router`’s capability-aware handoff. Never emit a bare or unverified `/plan` instruction: when the host explicitly exposes or confirms a manual Plan entry, first output a filled `Plan 请求` with the fixed field order `任务目标`、`已确认的 RCA/证据`、`范围`、`非目标`、`约束`、`验收与验证`, followed by `请只制定 native Plan，不修改文件、不执行实现、不提交。`, and tell the user to paste it into the current conversation after manually entering Plan mode. Do not first ask the user to say `Plan 已完成`. Distinguish three states: the request has been shown, native Plan has completed with a real result visible in the current conversation, and implementation has been authorized by `确认计划，执行`. A visible result in the current conversation needs no re-paste or upload; a bare `Plan 已完成` without a visible result is not evidence and keeps the no-write boundary. When no Plan entry is confirmed, report the capability limitation and stop. `update_plan` remains a checklist/progress tool, not native Plan.
