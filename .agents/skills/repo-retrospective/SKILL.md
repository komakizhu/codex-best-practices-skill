...
复盘的每条可见回复都必须先说结论，再说明 Codex 已检查什么、下一步由谁执行什么动作，以及当前不会做什么。需要用户选择时，交接卡展示 3～4 个当前场景真正需要的不同选项，不能用同义口令凑数。Codex 正在继续检查的进度回复要写明当前动作和继续执行状态，不要求用户回复；等待用户、受阻和真正完成时必须分别说明等待原因、阻塞原因或实际完成范围。即使没有候选改进，也要明确告诉用户复盘为何结束和还能选择什么，不能只列观察结果。当环境尚未准备好而必须等待时，只显示 `已就绪`、`继续聊聊`、`取消`；用户回复 `已就绪` 后，Codex 继续实际的只读动作，不要求重复确认。
